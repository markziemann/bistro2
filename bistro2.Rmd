...
tx_track <- autoplot(Homo.sapiens, which = wh)
...
meth_track <- autoplot( gene_data , which = generange, geom = "bar")
...
seqlevelsStyle(genes) = "UCSC"  # necessary
genes19 <- liftOver(genes, ch)
...
gr19$sig <- gsub("0","gray",gr19$sig)
gr19$sig <- gsub("1","red",gr19$sig)

meth_data <- gr19[queryHits(findOverlaps(gr19,generange))]

meth_track <- autoplot( meth_data , which = generange, geom = "bar", fill= meth_data$sig )
wh <- genesymbol[c("LEP")]+3000
wh <- range(wh, ignore.strand = TRUE)
#Plot the different transcripts  for our gene of interest
tx_track <- autoplot(Homo.sapiens, which = wh)
...
tx_track <- autoplot(Homo.sapiens, which = generange)
tracks(Genes=tx_track , Meth=meth_track, heights = c(0.1, 0.2)) + ylab("")
...
lapply(c(1,2,4:7), function(i) {
  ...
  SUBHEADER=paste("rho=",signif(ct$est,2), " p=",signif(ct$p.value,2),sep="")
