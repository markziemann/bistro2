...
ma <- read.table("fastq/methaverage.tsv")

ma$pool <- sapply(strsplit(ma$V1 , "\\."),"[[",1)

colnames(ma) <- c("sample","chr","CGn","CGb","CHGn","CHGb","CHHn","CHHb","CHn","CHb","Pool")

ma %>% kbl() %>% kable_styling()

```

Now run a spearman correlation test after correcting for CHH beta value (pool8 and pool17 still omitted).
...
chh <- ma[match(samplesheet$poolID, ma$Pool),]$CHHb
...
par(mfrow=c(1,2))
x <- lapply( 1:nrow(meth), function(i) {
  x <- as.vector(percMethylation(meth)[i,] - chh)
  cor <- cor.test(x,z,method="pearson")
  p <- cor$p.value
  r <- cor$estimate
  CHR=as.vector(seqnames(GRanges(meth)))[i]
